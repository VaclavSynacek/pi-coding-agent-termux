...
7. **Update package.json version and regenerate package-lock.json**
   - Version already updated during rebase conflict resolution in `packages/coding-agent/package.json`
   - Regenerate package-lock.json to sync with new version:
   ```bash
   rm package-lock.json
   npm install  # canvas will fail - this is expected and OK
   git add package-lock.json
   git commit -m "chore: Regenerate package-lock.json for v{VERSION}"
   git push origin master --force-with-lease
   ```
